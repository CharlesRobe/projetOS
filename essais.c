...
void simulateCars(int car_number, int seed_incrementer, int duration_minutes, int pipe_fd) {
...
int compareResultss(const void *a, const void *b) {
...
void runTrialsSession(const char *session_name, int duration_minutes, int amount_cars, int *car_numbers, CarResult *results) {
    ...
    for (int i = 0; i < amount_cars; i++) {
        ...
        if (pids[i] == 0) { // Code exécuté par le processus enfant.
            ...
            simulateCars(car_numbers[i], rand(), duration_minutes, pipes[i][1]); // Simulation de la voiture.
    ...
    qsort(results, amount_cars, sizeof(CarResult), compareResultss);
...
int essais() {
    ...
    runTrialsSession("P2", 60, NUM_CARS, car_numbers, results_P2);
    runTrialsSession("P3", 60, NUM_CARS, car_numbers, results_P3);
